...
class ConfigManager extends EventEmitter {
  ...
  decodeIdx(b64encoded) {
    return URLSafeBase64.decode(`${b64encoded}==`).reduce((output, elem) =>
  ...
  async loadConfiguration() {
    ...
    return new Promise((resolve, reject) => {
      readFile(`${isDevelopment ? process.cwd() : 'C:\\kiosk'}/config.json`, 'utf-8', (err, content) => {
        try {
