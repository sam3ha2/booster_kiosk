...
class VguangScannerOption {
    constructor (options = {}) {
        Object.keys(VguangScannerOption.Fields).forEach(k => {
            ...
            if (paramStruct.required) if (options[k] === undefined) throw `VguangScannerOption.fields[${k}] 매개변수는 필수이며, 생략할 수 없습니다!`;
...
VguangScannerOption.Fields = {
    ...
    mode: {
        ...
        type: String,
        check (value) {
            if (!(value in VguangScannerOption.Modes)) throw `mode는 VguangScannerOption.modes 내의 매개변수만 가능합니다!`;
        }
    },
}
